...
import 'package:sqlite_flutter/src/custom/validation.dart';
...
import 'package:sqlite_flutter/src/widgets/custom_input.dart';
...
class _FormCountryPageState extends State<FormCountryPage> {
  ...
  String mGenerateUniqueId() {
    // Generar UUID
    String mUuid = const Uuid().v4();

    // Obtener marca de tiempo actual
    String mTimestamp = DateFormat('yyyyMMddHHmmss').format(DateTime.now());

    // Generar número aleatorio de 6 dígitos
    String mRandomNumber = Random().nextInt(1000000).toString().padLeft(6, '0');

    // Combinar todo en un identificador único
    String mUniqueId = '$mUuid-$mTimestamp-$mRandomNumber';
      ...
      try {
        if (Provider.of<GlobalProvider>(context, listen: false).mCountry.mIdx == null) {
          ...
          await db.update(
              ...
              where: 'idx = ?',
              whereArgs: [Provider.of<GlobalProvider>(globalContext!, listen: false).mCountry.mIdx]);
          dialogDismiss();
  ...
  deleteCountry() async {
    try {
      if (Provider.of<GlobalProvider>(context, listen: false).mCountry.mIdx == null) {
        ...
        // Comentario: Elimina el país de la base de datos SQLite
        dialogDismiss();
